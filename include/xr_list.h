 ...
 * @brief iterate over list of given type
 * @param pos the type * to use as a loop counter.
 * @param db_type the type to use as a loop counter.
 * @param head the head for your list.
 * @param member the name of the list_struct within the struct.
 */
#ifdef  __cplusplus
#define list_for_each_entry_type(pos, db_type, head, member)				\
		for (pos = list_entry((head)->next, db_type, member);	\
			&pos->member != (head); 					\
			pos = list_entry(pos->member.next, db_type, member))
#else
#define list_for_each_entry_type(pos, db_type, head, member)				\
		for (pos = list_entry((head)->next, db_type, member),	\
			__builtin_prefetch(pos->member.next,0,1);			\
